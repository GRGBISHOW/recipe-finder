...
const Categories = () => {
  ...
  const getIngredientByCategory = async (categoryName) => {
    const response = await fetch(`http://localhost:3001/backend/category/${categoryName}`, {
      method: "GET",
    });
    const data = await response.json();
    dispatch(setCategories({ categories: data }));
                    ...
                    (ii) => {
                        return (                                                    
                            ...
                            <Box style={{ display:'flex', height:'92%' ,backgroundColor:'white',margin:'30px', borderRadius:'30px', flexDirection:'column' , boxShadow: '1px 1px 8px rgba(0,0,0,0.3'}}>
                                <Box
                                 key={ii.name} onClick={() => {dispatch(setSelectedCategory({selectedCategory: ii}));navigate(`/category/${ii.name}`)}}
                                 sx={{
                                  '&:hover':{
                                    cursor: 'pointer',
                                  }
